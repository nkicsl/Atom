...
func opMload(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	//fmt.Println("This is Mload, v:", v,"offset:",offset,callContext.memory.GetPtr(offset, 32))
	count++
	return nil, nil
}
...
func opMstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	switch val.Int64() {
		case 80:
			...
			vals1 :=  callContext.memory.GetPtr(160,32)
			vals2 :=  callContext.memory.GetPtr(224,32)
 			if reflect.DeepEqual(vals1, vals2){	
			}
			count++

		case 99:
			fmt.Println("This is Mstore under 99!","Start Time:",time.Now().UnixNano(),"Start Count:",count)
		case 100:
			fmt.Println("This is Mstore under 100!","Stop Time:",time.Now().UnixNano(),"Stop Count:",count)
