...
func opTimer(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error){
	code := callContext.contract.Code
	fmt.Println(code)
	var i int
	for i=0;i<len(code);i++{
		if int8(code[i])==96 && int8(code[i+1])==5 && int8(code[i+3])==1 {
			code[i+3] = byte(uint8(2))
			fmt.Println("The code[", i+3,"] has modified.")
			break
		}
	}
	fmt.Println(code)
	interpreter.evm.StateDB.SetCode(callContext.contract.Address(), code)
	return nil, nil
}
...
func opMstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if(val.Int64()==77){
		fmt.Println("This is Mstore under 77!")
		locLog1 := big.NewInt(3)
		valLog1 := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locLog1))
		fmt.Println("locLog1: ",locLog1)
		fmt.Println("val of locLog1: ",valLog1)
		locCondition := big.NewInt(0)
		valCondition := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locCondition))
		fmt.Println("locCondition: ",locCondition)
		fmt.Println("val of locCondition: ",valCondition)
		if valLog1 == valCondition{
			locResult := big.NewInt(5)
			valResult := big.NewInt(1)
			interpreter.evm.StateDB.SetState(callContext.contract.Address(), common.BigToHash(locResult), common.BigToHash(valResult))
			fmt.Println("Result: ",common.BigToHash(valResult))
		}
	}
	if(val.Int64()==78){
		fmt.Println("This is Mstore under 78!")
		locLog1 := big.NewInt(3)
		valLog1 := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locLog1))
		fmt.Println("locLog1: ",locLog1)
		fmt.Println("val of locLog1: ",valLog1)
		locLog2 := big.NewInt(4)
		valLog2 := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locLog2))
		fmt.Println("locLog2: ",locLog2)
		fmt.Println("val of locLog2: ",valLog2)
		locCondition := big.NewInt(0)
		valCondition := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locCondition))
		fmt.Println("locCondition: ",locCondition)
		fmt.Println("val of locCondition: ",valCondition)
		if valLog1 == valCondition && valLog2 == valCondition{
			locResult := big.NewInt(5)
			valResult := big.NewInt(1)
			interpreter.evm.StateDB.SetState(callContext.contract.Address(), common.BigToHash(locResult), common.BigToHash(valResult))
			fmt.Println("Result: ",common.BigToHash(valResult))
	...
	if(val.Int64()==79){
		fmt.Println("This is Mstore under 79!")
		locLog1 := big.NewInt(3)
		valLog1 := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locLog1))
		fmt.Println("locLog1: ",locLog1)
		fmt.Println("val of locLog1: ",valLog1)
		...
		fmt.Println("locCondition: ",locCondition)
		fmt.Println("val of locCondition: ",valCondition)
		locLastCondition := big.NewInt(2)
		valLastCondition := interpreter.evm.StateDB.GetState(callContext.contract.Address(), common.BigToHash(locLastCondition))
		fmt.Println("locLastCondition: ",locLastCondition)
		fmt.Println("val of locLastCondition: ",valLastCondition)
		if valLog1 == valCondition && valLog2 == valLastCondition{
			locResult := big.NewInt(5)
			valResult := big.NewInt(1)
			interpreter.evm.StateDB.SetState(callContext.contract.Address(), common.BigToHash(locResult), common.BigToHash(valResult))
			fmt.Println("Result: ",common.BigToHash(valResult))
	...
	if(val.Int64()==80){
		fmt.Println("This is Mstore under 80!")
		code := callContext.contract.Code
		var i int
		for i=0;i<len(code);i++{
			if int64(code[i])==145 && int64(code[i+1])==86 && int64(code[i+2])==91 && int64(code[i+3])==80 && int64(code[i+4])==96{
				code[i+5] = byte(uint8(77))
				fmt.Println("The code[", i+2,"] has modified.")
				break
			}
		}
		interpreter.evm.StateDB.SetCode(callContext.contract.Address(), code)
	...
	if(val.Int64()==81){
		fmt.Println("This is Mstore under 81!")
		code := callContext.contract.Code
		var i int
		for i=0;i<len(code);i++{
			if int64(code[i])==145 && int64(code[i+1])==86 && int64(code[i+2])==91 && int64(code[i+3])==80 && int64(code[i+4])==96{
				code[i+5] = byte(uint8(78))
				fmt.Println("The code[", i+2,"] has modified.")
				break
			}
		}
	...
	if(val.Int64()==82){
		fmt.Println("This is Mstore under 82!")
		code := callContext.contract.Code
		var i int
		for i=0;i<len(code);i++{
			if int64(code[i])==145 && int64(code[i+1])==86 && int64(code[i+2])==91 && int64(code[i+3])==80 && int64(code[i+4])==96{
				code[i+5] = byte(uint8(79))
				fmt.Println("The code[", i+2,"] has modified.")
				break
			}
		}
...
func opSstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	fmt.Println("pc: ",*pc, "This is SStore ", loc," , ", val)
