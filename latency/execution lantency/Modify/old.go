...
func opMstore(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	if(val.Int64()==77){
		...
		policy_test := []byte("This is fyz's first policy!")
		interpreter.evm.StateDB.SetPolicy(callContext.contract.Address(), policy_test)
		fmt.Println(interpreter.evm.StateDB.GetPolicyCode(callContext.contract.Address()))
		fmt.Println("The policy is added into stateDB!")
	}
	if(val.Int64()==78){
		fmt.Println("This is Mstore under 78!")
		ret := interpreter.evm.StateDB.GetPolicyCode(callContext.contract.Address())
		fmt.Println(ret)
	}
	if(val.Int64()==79){
		...
		var i int
		for i=0;i<len(code);i++{
			if int64(code[i])==84 && int64(code[i+1])==130 {
				code[i+2] = byte(uint8(20))
